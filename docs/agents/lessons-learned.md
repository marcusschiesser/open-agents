...
- Do not switch local Daytona default snapshots to `daytonaio/sandbox:*‑slim`; those images can create sandboxes, but they do not include Node.js/package-manager tooling, so the repo dev-server flow fails for Vite/Next/npm/bun projects even after sandbox creation succeeds.
